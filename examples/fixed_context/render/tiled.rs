...
pub fn render_tiled(dcx: &mut DrawContext, map: &Map, texture: &TextureData2d, bounds_px: Rect2i) {
    let left_up = {
        let mut pos = w2t(bounds_px.left_up(), map);
    ...
    let right_down = {
        let mut pos = w2t(bounds_px.right_down(), map);
        ...
        for y in (left_up.y as usize)..(right_down.y as usize) {
            for x in (left_up.x as usize)..(right_down.x as usize) {
                ...
                pass.texture(texture)
                    ...
                    .dest_rect_px([
                        (
                            (x as u32 * tile_size.x - bounds_px.left_up().x as u32) as f32,
                            (y as u32 * tile_size.y - bounds_px.left_up().y as u32) as f32,
                        ),
